...
<%
	...
	String nuevo_nombre= (String)request.getParameter("nnombre");
	
	
	if(pass.equals(pass_ingresada)){
		ClientUsuario w = new ClientUsuario();
		if(w.modificarNombre(iduser, nuevo_nombre) == 1){
			response.sendRedirect("user_modify2_ok.jsp");
		}
		else{
			response.sendRedirect("user_modify2_nok.jsp");
		}
	}
	else{
		response.sendRedirect("user_modify2_nok.jsp");
